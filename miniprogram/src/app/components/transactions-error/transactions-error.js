...
Component({
  ...
  didMount() {
    ...
    this.$page.currentTranErrorCom = this
    my.tranErrorFunc = (obj) => {
      // for 503 http status
      ...
      const _this_ = getCurrentPages()[getCurrentPages().length - 1].currentTranErrorCom
      my.call("enableSwipe", { isSwipe: false }, () => {})
      let errorButton = ''
      let event = ''
      let errorTitle = obj.errorCode + '_errorTitle'
      let errorMessage = obj.errorCode + '_errorMessage'
      if (obj.errorHttpStatus === 409 && obj.errorCode === 'TRX-000-608') {
        errorButton = 'transactionsErrorResponseToSplashButton'
