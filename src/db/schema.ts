...
export const events = sqliteTable("events", {
  ...
  isActive: integer("is_active").default(0), // 0 = inactive, 1 = active (admin only)
...
export const runningEvents = sqliteTable("running_events", {
  ...
  paymentMethod: text("payment_method"), // biller | transfer
...
export const runningEventsRelations = relations(runningEvents, ({ many, one }) => ({
  ...
  user: one(users, {
    fields: [runningEvents.userId],
    references: [users.id],
  }),
}));

export const runningCategoriesRelations = relations(runningCategories, ({ one, many }) => ({
  runningEvent: one(runningEvents, {
    fields: [runningCategories.runningEventId],
    references: [runningEvents.id],
  }),
  tickets: many(runningTickets),
}));

export const runningTicketsRelations = relations(runningTickets, ({ one }) => ({
  category: one(runningCategories, {
    fields: [runningTickets.categoryId],
    references: [runningCategories.id],
  }),
}));
