      ...
      <div className={`bg-white border-r border-gray-200 min-h-screen flex flex-col fixed left-0 top-0 bottom-0 z-30 transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'} shadow-lg`}>
        ...
        <div className="flex-1 p-4 overflow-x-hidden overflow-y-auto">
          ...
          <nav className="space-y-1">
              <Link 
              to="/" 
              className={`group flex items-center py-2 text-sm font-medium rounded-md text-gray-600 hover:bg-gray-50 hover:text-gray-900 ${isCollapsed ? 'justify-center px-0' : 'px-2'}`}
              activeProps={{ className: '!bg-orange-50 !text-orange-600' }}
              activeOptions={{ exact: true }}
              title="Events"
              >
              <span className={`h-6 w-6 text-gray-400 group-hover:text-gray-500 group-active:text-orange-500 flex items-center justify-center ${!isCollapsed && 'mr-3'}`}>
                  📅
              </span>
