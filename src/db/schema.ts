...
export type ApprovalStatus = "approved" | "requested" | "rejected";
...
export const events = sqliteTable("events", {
  id: text("id").primaryKey(),
  name: text("name").notNull(),
  eventType: text("event_type").default("internal"),
...
export const runningEvents = sqliteTable("running_events", {
  id: text("id").primaryKey(),
  name: text("name").notNull(),
  eventType: text("event_type").default("internal"),
  externalUrl: text("external_url"),
...
export const runningCategories = sqliteTable("running_categories", {
  id: text("id").primaryKey(),
  runningEventId: text("running_event_id").notNull().references(() => runningEvents.id, { onDelete: 'cascade' }),
  name: text("name").notNull(),
  description: text("description"),
  price: integer("price").notNull(),
  maxPrice: integer("max_price"),
...
export const runningTickets = sqliteTable("running_tickets", {
  id: text("id").primaryKey(),
  categoryId: text("category_id").notNull().references(() => runningCategories.id, { onDelete: 'cascade' }),
  name: text("name").notNull(),
  description: text("description"),
  type: text("type").default("normal"),
  price: integer("price").notNull(),
...
export const serviceOrders = sqliteTable("service_orders", {
  ...
  paymentMethod: text("payment_type").default("va"),
...
export const runningEventsRelations = relations(runningEvents, ({ many, one }) => ({
  categories: many(runningCategories),
  user: one(users, { fields: [runningEvents.userId], references: [users.id] }),
}));

export const runningCategoriesRelations = relations(runningCategories, ({ one, many }) => ({
  runningEvent: one(runningEvents, { fields: [runningCategories.runningEventId], references: [runningEvents.id] }),
  tickets: many(runningTickets),
}));

export const runningTicketsRelations = relations(runningTickets, ({ one }) => ({
  category: one(runningCategories, { fields: [runningTickets.categoryId], references: [runningCategories.id] }),
}));
...
export type RunningCategory = typeof runningCategories.$inferSelect;
export type NewRunningCategory = typeof runningCategories.$inferInsert;
export type RunningTicket = typeof runningTickets.$inferSelect;
export type NewRunningTicket = typeof runningTickets.$inferInsert;
